...
def SetEnvironmentAndGetRuntimeDllDirs():
  ...
  if ((_HostIsWindows() or os.path.exists(json_data_file))
    ...
    if not 'GYP_MSVS_OVERRIDE_PATH' in os.environ:
      ...
      print(f"DEBUG: GYP_MSVS_OVERRIDE_PATH set to {os.environ['GYP_MSVS_OVERRIDE_PATH']}", file=sys.stderr)
...
def GetVisualStudioVersion():
  ...
  for version in supported_versions:
    ...
    print(f"DEBUG: Checking version {version}, env var val={repr(path)}", file=sys.stderr)
    if path and os.path.exists(path):
      available_versions.append(version)
      break
    # Detecting VS under possible paths.
    if version >= '2022' or version == '18':
    ...
    print(f"DEBUG: Checking FS path for {version}: {path}", file=sys.stderr)
    if path and any(
        os.path.exists(os.path.join(path, edition))
        for edition in ('Enterprise', 'Professional', 'Community', 'Preview',
                        'BuildTools')):
      print(f"DEBUG: Found valid edition for {version} at {path}", file=sys.stderr)
      available_versions.append(version)
      break
    print(f"DEBUG: No valid edition found for {version} at {path}", file=sys.stderr)

  if not available_versions:
    raise Exception('No supported Visual Studio can be found.'
                    ' Supported versions are: %s.' % supported_versions_str)
  print(f"DEBUG: Selected version: {available_versions[0]}", file=sys.stderr)
...
def DetectVisualStudioPath():
  ...
  if version_as_year >= '2022' or version_as_year == '18':
    program_files_path_variable = '%ProgramFiles%'
  else:
    program_files_path_variable = '%ProgramFiles(x86)%'
  for path in (os.environ.get('vs%s_install' % version_as_year),
               ...
               os.path.expandvars(program_files_path_variable +
                                  ...
                                  version_as_year)):
    if path and os.path.exists(path):
      print(f"DEBUG: DetectVS returning {path}", file=sys.stderr)
      return path
